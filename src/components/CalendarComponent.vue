<template>
    <div class="row">
        ...
        <div
            class="col-11 m-0 position-relative input-col-10-padding"
            :data-bs-toggle="'modal'"
            :data-bs-target="'#picker-frequency'"
        >
            <input
                class="custom-select-frequency"
    ...
    <ModalComponent
        :id-data="'picker-frequency'"
        :modal-title="'Pasirinkite kelionės dažnumą'"
    >
        <template #body-content>
            <div
                class="modal-calendar-container d-flex flex-column align-items-center"
            >
                ...
                <div class="col-5">
                    <ButtonComponent
                        @click="submit"
                        class="order-form-submit-button m-0 w-100"
                    >
                        <template #btnContent> Select Date </template>
                    </ButtonComponent>
                </div>
            </div>
            ...
            <template v-if="openCustomModal">
                <div class="container">
                    ...
                    <button
                        type="submit"
                        @click="calculateAllSelectedDates(this.selectedDates, this.selectedPeriodType, this.selectedDays)">
                        <p class="m-0">
                            Repeat MULTIPLE <span>{{ period }}</span> Selected
                            <span>{{ selectedPeriodType }}</span>
                        </p>
                    </button>
                </div>
            </template>
    ...
    <Teleport to="#modals">
        <div
            v-if="openPredifinedModal"
            class="position-absolute predifined-select-modal"
            id="predifined-select-modal"
        >
            <select
                :value="selectedPredifinedOption"
                @change.prevent="selectPredifinedOption($event)"
                class="container__selected"
            >
                ...
                <option :value="'weekly'">
                    Weekly on
                    {{ this.weekDays[this.selectedDates[0].getDay()].name }}
                </option>
                <option :value="'monthly'">
                    Monthly on the {{ this.nthSelectedDay + 1 }}th {{ this.weekDays[this.selectedDates[0].getDay()].name }} 
                </option>
                <option :value="'annually'">Annually on {{ this.months[this.selectedDates[0].getMonth()] }} {{ this.selectedDates[0].getDate() }}</option>
            ...
            <button
                @click="
                    close({
                        id: 'predifined-select-modal',
                        backdrop: 'custom-modal-backdrop-0',
...
export default {
    ...
    methods: {
        ...
        selectPredifinedOption(event) {
            ...
            switch (event.target.value) {
                ...
                case 'daily':
                    ...
                    newDates = this.calculateDates(1, startDate, endDate, 1, 'day');
                    break

                case 'weekly':
                    endDate = new Date(startDate.getFullYear() + 14, startDate.getMonth(), startDate.getDate());
                    newDates = this.calculateDates(1, startDate, endDate, 7, 'week');
                    break

                case 'monthly':
                    endDate = new Date(startDate.getFullYear() + 18, startDate.getMonth(), startDate.getDate());
                    newDates = this.calculateAllNthDays(startDate, endDate, this.nthSelectedDay, this.selectedDays[0]);
                    break

                case 'annually':
                    endDate = new Date(startDate.getFullYear() + 78, startDate.getMonth(), startDate.getDate());
                    newDates = this.calculateDates(1, startDate, endDate, 0, 'year');
                    console.log(event.target.value, newDates);
                    break

                case 'weekday':
                    endDate = new Date(startDate.getFullYear() + 3, startDate.getMonth(), startDate.getDate());
                    newDates = this.calculateDates(1, startDate, endDate, 1, 'day').filter(el => el.getDay() !== 0).filter(el => el.getDay() !== 6);
                    // newDates = [...newDates];
                    // result = newDates;
        ...
        selectDate(event) {
            ...
            for (let i = 0; i < this.daysInWeek; i++) {
                for (const days of this.weekDays) {
                    if (days.id === new Date(new Date(this.selectedDates[0].getTime() + i * this.dayInMiliseconds).toDateString()).getDay()) {
                        days.date = new Date(new Date(this.selectedDates[0].getTime() + i * this.dayInMiliseconds).toDateString());
                    }
                }
            }

            this.nthSelectedDay = this.calculateNthSelectedDay(this.selectedDates[0]);
        ...
        calculateDates(period, date, endDate, multiplier, selectedPeriodType) {
            ...
            switch (selectedPeriodType) {
                ...
                case 'week':
                    ...
                    break
                case 'month':
                    deadline = Math.ceil(end.getMonth() - start.getMonth() + this.monthsInYear * (end.getFullYear() - start.getFullYear()));
        ...
        calculateAllDates(period, dates, endDate, multiplier, selectedPeriodType) {
            let result = [];
            for (const date of dates) {
                let tempArr = this.calculateDates(period, date, endDate, multiplier, selectedPeriodType);
                    result.push(...tempArr);
            }

            return result;
        },
        calculateAllSelectedDates(dates, selectedPeriodType, selectedDays) {
            const multipleDays = [];
            ...
            switch (selectedPeriodType) {
                case 'day':
                    multiplier = 1
                    selectedDatesTemp = this.calculateAllDates(this.period, dates, this.endDate, multiplier, selectedPeriodType);

                    break
                case 'week':
                    multiplier = 7
                    multipleDays.push(...this.getDatesOfSelectedDays( this.selectedDays, this.weekDays));
                    selectedDatesTemp = this.calculateAllDates(this.period, multipleDays, this.endDate, multiplier, selectedPeriodType);
            ...
            selectedDatesTemp = [
                ...new Set(selectedDatesTemp.map((el) => el.toISOString()))
            ]
            this.selectedDates = [
                ...selectedDatesTemp.map((el) => new Date(el))
            ]
        },
        calculateNthSelectedDay(date) {
            let start = new Date(date.toDateString());
            ...
            for (let j = 0; j < daysCount; j++) {
                if (new Date(start.getFullYear(), start.getMonth(), j + 1).getDay() === start.getDay()) {
                    monthDaysArr.push({
                        date: new Date(start.getFullYear(), start.getMonth(), j + 1),
                        day: new Date(start.getFullYear(), start.getMonth(), j + 1).getDay()
                    })
                }
            }
        ...
        calculateAllNthDays(date, endDate, nthDay, day) {
            let start = new Date(date.toDateString());
            let interval = endDate.getMonth() - start.getMonth() + this.monthsInYear * (endDate.getFullYear() - start.getFullYear());
            let calculatedDays = [];
            ...
            for (let i = 0; i < interval; i++) {
                ...
                for (let j = 0; j < daysCount; j++) {
                    if (new Date(fullDate.getFullYear(), month, j + 1).getDay() === day) {
                        tempArr.push({
                            date: new Date(fullDate.getFullYear(), month, j + 1),
                            day: new Date(fullDate.getFullYear(), month, j + 1).getDay()
                        });
                    }
                }
            ...
            return calculatedDays.map((el) => el[nthDay].date);
        },
